...
## Settings/config screen with audio, gameplay, and system options.
## Port of SettingsScene from App.tsx — redesigned with section headers,
## custom sliders, row dividers, and proper focus highlighting.
extends Control
...
var _all_rows: Array[Dictionary] = []       # combined list of {type, ...}
...
func _create_section_header(data: Dictionary) -> Control:
	...
	# Extra top margin for all but first section
	if not is_first:
		var spacer := Control.new()
		spacer.custom_minimum_size = Vector2(0, 24)
		container.add_child(spacer)

	# White 8x8 indicator dot
...
func _create_config_row(index: int, cfg: Dictionary) -> Control:
	...
	# --- Highlight background (appears on focus) ---
	var highlight := ColorRect.new()
	highlight.name = "Highlight"
	highlight.set_anchors_preset(Control.PRESET_FULL_RECT)
	highlight.color = Color(1, 1, 1, 0.0)   # invisible until focused
...
# -------------------------------------------------------------------
# Custom slider (web-style: 4px track, 24px thumb, glow)
# -------------------------------------------------------------------
...
func _create_slider_control(parent: Control, cfg: Dictionary) -> void:
	...
	# Thumb (24x24, white border, black fill, glow shadow)
	# We simulate the glow with a larger, softer white ColorRect behind the thumb
	...
	# Invisible HSlider for actual interaction
	...
	slider.modulate.a = 0.0   # invisible, only for input
	# Use flat style to avoid default theme drawing
...
func _refresh_display_texts() -> void:
	# Rebuild row data with new language
	_build_rows()
	# Update label texts on existing row nodes
		...
		for entry: Dictionary in _all_rows:
			if entry.type == "row":
				if row_idx == i:
					...
					if zh: zh.text = entry.zh
					if en: en.text = entry.label
					break
				row_idx += 1
	_update_display_values()
...
func _update_row_focus() -> void:
	for i: int in range(_row_nodes.size()):
		...
		# Text colors
		zh.add_theme_color_override("font_color", Color.WHITE if is_focused else Color(1, 1, 1, 0.6))
		en.add_theme_color_override("font_color", Color(1, 1, 1, 0.4) if is_focused else Color(1, 1, 1, 0.2))

		# Value label boldness on focus
...
# ===================================================================
# Back button bar (matches web version SceneLayout)
# ===================================================================
...
func _setup_back_button() -> void:
	...
	# ESC box (48x48, white bg, black "ESC" text)
	...
	# Interaction — click anywhere on the bar to go back
...
func _input(event: InputEvent) -> void:
	...
	if event.is_action_pressed("ui_up"):
		...
		# Find the row data for current focus
