## ScenesData : RefCounted
## 场景/背景画廊数据（旧实现 — 暂时无关，保留待用）。
## 扫描 assets/backgrounds/scenes/ 并按文件名前缀分组。
## 当前画廊页面使用的平面列表数据见 SceneGalleryData.gd。
##
...
static func _scan_dir(dir_path: String, ext: String) -> Array[String]:
	...
	if not da:
		push_warning("ScenesData: Cannot open directory — ", dir_path)
		return files
